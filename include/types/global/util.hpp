...
template <int d>
using make_dim_t = std::conditional_t<rdim<d>, runtime_dim, compiled_dim<d>>;
template <int d>
using make_arg_t = std::conditional_t<rdim<d>, size_t, std::nullptr_t>; // we pass runtime dims as size_t, comptime dims as nullptr_t
} // namespace gl
} // namespace ducks

namespace detail {
template <typename T>
concept tile = ducks::rt::all<T> || ducks::rt_base::all<T>;
// concept tile = ducks::st::all<T> || ducks::rt::all<T> || ducks::cst::all<T> || ducks::crt::all<T>;
// TODO: template<typename T> concept vec  = ducks::sv::all<T> || ducks::rv::all<T> || ducks::csv::all<T> || ducks::crv::all<T>;
} // namespace detail
...
template <typename _T = ducks::default_type>
struct coord { // essentially a named int4 for tensor coordinates.
  ...
  template <int row_axis, int col_axis>
  __device__ inline coord<ducks::default_type> unit_coord() const {
  ...
  template <int axis>
  __device__ inline int dim() const {

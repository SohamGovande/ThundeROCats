...
// Don't use constexpr for now because it's not supported by certain operations
#define todo_constexpr
...
__host__ __device__ inline __hip_bfloat16 __float2bfloat16_rn(const float a) {
...
// static_assert(sizeof(bf16) != sizeof(bf16_2), "bf16 and bf16_2 must have different sizes");
/**
...
struct constants<float2> {
  static __device__ inline todo_constexpr float2 zero() { return float2{0.f, 0.f}; }
  static __device__ inline todo_constexpr float2 one() { return float2{1.f, 1.f}; }
  static __device__ inline todo_constexpr float2 pos_infty() { return float2{constants<float>::pos_infty(), constants<float>::pos_infty()}; }
  static __device__ inline todo_constexpr float2 neg_infty() { return float2{constants<float>::neg_infty(), constants<float>::neg_infty()}; }
};
template <>
...
struct constants<half_2> {
  static __device__ inline todo_constexpr half_2 zero() { return half_2{constants<half>::zero(), constants<half>::zero()}; }
  static __device__ inline todo_constexpr half_2 one() { return half_2{constants<half>::one(), constants<half>::one()}; }
  static __device__ inline todo_constexpr half_2 pos_infty() { return half_2{constants<half>::pos_infty(), constants<half>::pos_infty()}; }
  static __device__ inline todo_constexpr half_2 neg_infty() { return half_2{constants<half>::neg_infty(), constants<half>::neg_infty()}; }
};
...
struct constants<int2> {
  static __device__ inline todo_constexpr int2 zero() { return int2{0, 0}; }
  static __device__ inline todo_constexpr int2 one() { return int2{1, 1}; }
};
...
struct packing {
   ...
   */
  static __device__ inline todo_constexpr T pack(const bf16 &i);
};
template <>
struct packing<bf16> {
  static __device__ inline constexpr int num() { return 1; }
